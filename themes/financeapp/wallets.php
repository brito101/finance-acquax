...
<section class="app_wallets">
    <article class="wallet wallet_add radius gradient-blue">
        ...
        <h3>Crie e gerencie suas empresas</h3>
        ...
        <span class="btn wallet_action radius transition icon-plus-circle">Adicionar Empresa</span>

        <div class="wallet_overlay radius" style="background-color: var(--color-blue)">
            <div>
                <p>Insira o nome da sua nova empresa, como <b>Minha Casa</b>, <b>Minha Empresa</b>, <b>Cartão 5546</b>...
                    e tudo pronto!</p>
    ...
    <?php foreach ($wallets as $wallet) : $balance = $wallet->balance() ?>
        <article class="wallet radius <?= ($balance->balance == "positive" ? "gradient-green" : "gradient-red"); ?>">
            ...
            <input data-walletedit="<?= url("/app/wallets/{$wallet->id}"); ?>" type="text" name="wallet" value="<?= $wallet->wallet; ?>">
            ...
            <div class="wallet_overlay radius">
                <div>
                    ...
                    <p>Ao deletar uma empresa todos as contas lançadas nesta serão perdidas. Tem certeza que deseja
                        deletar a empresa?</p>
                    <span data-walletremove="<?= url("/app/wallets/{$wallet->id}"); ?>" data-wallet="<?= $wallet->id; ?>" class="btn radius transition icon-warning">Sim, DELETAR!</span>

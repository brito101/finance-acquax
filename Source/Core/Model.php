...
abstract class Model
{
    ...
    public function distinctFind(string $columns, ?string $terms = null)
    {
        if ($terms) {
            $this->query = "SELECT DISTINCT {$columns} FROM {$this->entity} WHERE {$terms}";
        }

        $this->query = "SELECT DISTINCT {$columns} FROM {$this->entity}";
        return $this;
    }
